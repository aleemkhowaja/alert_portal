...
function screenGeneratorProp_openQueryScreen(elementId,rowId,elementType,elementProperty,sourceMapping)
{
	...
	var buttonsArr = {};
	buttonsArr[ok_label_trans] = function() {
		optionGrid_saveQueryDetails($(this),rowId,elementProperty,screenId,elementType);
	};
 ...
 * @returns
 */
function optionGrid_saveQueryDetails(theDialog,rowId,elementProperty,screenId,elementType)
{
...
{
	var sourceMappingParam=null;
	if(rowObject["COL_IS_LIVESEARCH"] == undefined || rowObject["COL_IS_LIVESEARCH"] < 1)
	{
		//Bug#987227 Error in Livesearch of Colum Properties Grid
		if (rowObject["SOURCE_MAPPING_CODE"]!=undefined)
		{
			sourceMappingParam = rowObject["SOURCE_MAPPING_CODE"];
		}
		return  '<button disabled type="button" onclick="screenGeneratorProp_openQueryScreen(\''+$("#elementId").val()+'\','+options.rowId+','+'null,null,'+sourceMappingParam+')"'+
	...
	{
		return  '<button type="button" onclick="screenGeneratorProp_openQueryScreen(\''+$("#elementId").val()+'\','+options.rowId+','+'null,null,'+sourceMappingParam+')"'+
...
function returnLookupScreenParamMap(lookupid)
{
	...
	if($('options_' + lookupid + '_liveSearch').length==0)
	{
